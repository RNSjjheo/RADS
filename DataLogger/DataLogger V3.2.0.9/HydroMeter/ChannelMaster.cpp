...
//---------------------------------------------------------------------------
// Init
void TChannelMaster::Init(TApdComPort *pPort, bool bFirst)
{
	m_bFirst = bFirst;
	m_RHydroMeter.First = bFirst;	// HydroMeter1 or not
	//m_RHydroCellContainer.First = bFirst;	// HydroMeter1Cell or not
...
bool TChannelMaster::Decode(AnsiString sCmd, int iLen)
{
	...
	if ( sCmd == "CS" ) {
		...
		if ( m_bFirst ) FormMain->SetHydroMeter1Icon(enHydroMeterAni1);
...
bool TChannelMaster::Receive(AnsiString RecvMsg, int iLen)
{
	...
	if ( iPos == 0 ) {
		...
		if ( iPos > 0 ) {
			...
			if ( Pos > 0 ) {
				...
				LogMessage("[[CS]ChannelMaster" + UnicodeString((m_bFirst) ? "1":"2") + "]<=" + sCmd);
			}
		}
	} else {
		br = bn = true;	// \r\n모두 있는 상황
	...
	while ( iPos > 0 ) {
		if ( iPos > 0 ) {	// \r\n
			...
			LogMessage("[ChannelMaster" + UnicodeString((m_bFirst) ? "1":"2") + "]<=" + sCmd);
		}
