...
//---------------------------------------------------------------------------
// Constructor
TRHydroMeterCell::TRHydroMeterCell(bool bFirst)
{
	First       = bFirst;
	MeasureDate = "";
	MeasureTime = "";
...
{
	return ((First) ? "RHydroMeter1Cell" : "RHydroMeter2Cell");
}
...
{
	if ( m_pList != NULL ) {
		m_bFirst = pRHydroMeterCell->First;
...
bool TRHydroMeterCellContainer::Save(UnicodeString MeasureDate, UnicodeString MeasureTime)
{
	...
	RHydroMeterCell.First       = m_bFirst;
	...
	sSQL = " INSERT INTO " + (( m_bFirst ) ? UnicodeString("RHydroMeter1Cell") : UnicodeString("RHydroMeter2Cell")) +
		   " ( MeasureDate, MeasureTime, CellNo, " +
