...
bool TRWaterLevel::Insert()
{
	...
	UnicodeString sSQL =
		" INSERT INTO RWaterLevel "
		" ( MeasureDate,  MeasureTime,  WaterKind,       "
		"   AvgWaterLevel, MinWaterLevel, MaxWaterLevel, "
		...
		"          '" + MeasureTime     + "', " +
		"          '" + WaterKind       + "', " +
		"           " + FloatToStr((Extended)AvgWaterLevel) + " , " +
		"           " + FloatToStr((Extended)MinWaterLevel) + " , " +
		"           " + FloatToStr((Extended)MaxWaterLevel) + " , " +
...
bool TRWaterLevel::Update()
{
	...
	UnicodeString sSQL =
		" UPDATE RWaterLevel "
		" SET WaterKind = '" + WaterKind                     + "', " +
		"     AvgWaterLevel =  " + FloatToStr((Extended)AvgWaterLevel) + " , " +
		"     MinWaterLevel =  " + FloatToStr((Extended)MinWaterLevel) + " , " +
		"     MaxWaterLevel =  " + FloatToStr((Extended)MaxWaterLevel) + " , " +
...
//---------------------------------------------------------------------------
bool TRWaterLevel::IsExist()
{
	bool bResult = false;

	UnicodeString sSQL =
		" SELECT COUNT(*) AS CNT FROM RWaterLevel    "
		" WHERE MeasureDate   = '" + MeasureDate     + "' AND " +
		"       MeasureTime   = '" + MeasureTime     + "'     ";
...
//---------------------------------------------------------------------------
bool TRWaterLevel::Check(int iAction)
{
	bool bResult = false;
...
//---------------------------------------------------------------------------
bool TRWaterLevel::GetData()
{
	UnicodeString sSQL;

	sSQL = 	" SELECT * FROM RWaterLevel "
			" WHERE MeasureDate   = '" + MeasureDate     + "' AND " +
			"       MeasureTime   = '" + MeasureTime     + "'     ";
	...
	while ( !pQuery->Eof ) {
		try {
			...
			WaterKind        = pQuery->FieldByName("WaterKind")->AsString;
			AvgWaterLevel    = pQuery->FieldByName("AvgWaterLevel")->AsFloat;
			MinWaterLevel    = pQuery->FieldByName("MinWaterLevel")->AsFloat;
			MaxWaterLevel    = pQuery->FieldByName("MaxWaterLevel")->AsFloat;
