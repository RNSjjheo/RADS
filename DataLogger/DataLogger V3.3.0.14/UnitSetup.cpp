...
void TFormSetup::ClearLogger()
{
	...
	// Hydro1 --------------------------------------------------------------
	cbxHydro1Type->ItemIndex     = 0;	// NONE
	...
	edHydro1Blank->Text          = "";
	// Airmar
	cbxAirmar1ComPort->ItemIndex = 0;	// NONE
	cbxAirmar1BaudRate->ItemIndex= 0;	// NONE
	cbAirmarLinker->Checked      = false;	// Default: Airmar Sensor , checed => use AirmarLinker
	cbxHydro1Reverse->ItemIndex  = -1;
	cbxHydro1IdleCheck->ItemIndex= 0;
	cbxHydro1IdleCheckTime->ItemIndex = 0;
	...
	// EWSV  2026.01.22
	cbxEWSVAvg->ItemIndex        = -1;
	edEWSVWorkTime->Text         =  0;
	cbxEWSVSensorAvg->ItemIndex  = -1;

	// Hydor2
	cbxHydro2Type->ItemIndex     =  0;  // NONE
	...
	cbxHydro2IdleCheck->ItemIndex = 0;
	cbxHydro2IdleCheckTime->ItemIndex = 0;
	edHydro2StartCell->Text      = "";
	edHydro2EndCell->Text        = "";

	// Hydor3
	cbxHydro3Type->ItemIndex     = 0;	// NONE
	cbxHydro3ComPort->ItemIndex  = 0;	// NONE
	cbxHydro3BaudRate->ItemIndex = 0;	// NONE
	edHydro3Offset->Text         = "0.0";
	edHydro3Hz->Text             = "";
	edHydro3SN->Text             = "";
	edHydro3CellNo->Text         = "";
	edHydro3CellSize->Text       = "";
	edHydro3Blank->Text          = "";
	cbxHydro3Reverse->ItemIndex  = -1;
	cbxHydro3UpSide->ItemIndex   = 1;
	cbxHydro3IdleCheck->ItemIndex = 0;
	cbxHydro3IdleCheckTime->ItemIndex = 0;
	edHydro3StartCell->Text      = "";
	edHydro3EndCell->Text        = "";

	// WaterLevel
	cbxWaterType->ItemIndex          = 0;		// 수위계 종류 NONE
	cbxWaterComPort->ItemIndex       = 0;		// 수위계 포트 NONE
	cbxWaterBaudRate->ItemIndex      = 0;		// 수위계 속도 NONE
	edWaterOffset->Text              = "";		// 수위계 오프셋
	edWaterSN->Text                  = "";		// 수위께 장비번호
	cxWaterAtmos->Checked            = false;	// (대기압)대기압 보정 유무
	cbxAtmosPort->ItemIndex          = 0;		// (대기압)대기압센서 포트 NONE
	edBaseAtmos->Text                = "";		// (대기압)기준대기압
	cxWaterAirmar->Checked           = false;	// (Airmar)기상센서 유무
	cbxWaterIdleCheck->ItemIndex     = 0;		// Idle Check On/Off
	cbxWaterIdleCheckTime->ItemIndex = 0;		// Idle Check Time
	cbxWaterUseFlag->ItemIndex       = 2;		// default: 수위계의 수위 데이터 사용
												// ( 1: 유속계 1의 수위 사용, 2: 수위계의 수위 사용)
}
...
bool TFormSetup::CheckLogger()
{
	...
	bOk = true;
	if ( cbxHydro1Type->ItemIndex == -1 )     bOk = false;
	if ( cbxHydro1Type->ItemIndex > 0 ) {	// NONE이 아니면 포트와 속도 지정
		if ( cbxHydro1ComPort->ItemIndex <= 0 )  bOk = false;
		if ( cbxHydro1BaudRate->ItemIndex <= 0 ) bOk = false;
	}
	//if ( edHydro1Offset->Text.IsEmpty() )     return false;
	if ( cbxHydro1Reverse->ItemIndex == -1 )  bOk = false;
	if ( cbxHydro1IdleCheck->ItemIndex == -1 ) bOk = false;
	if ( cbxHydro1IdleCheckTime->ItemIndex == -1 ) bOk = false;
	//if ( edHydro1StartCell->Text.IsEmpty() )  bOk = false;
	//if ( edHydro1EndCell->Text.IsEmpty() )    bOk = false;
	if ( !bOk ) {
		ShowMessage("유속계 1번의 설정에 오류가 있습니다.");
		return false;
	}

	// Check AG/SL Series
	bOk = true;
	if ( cbxHydro1Type->ItemIndex == SL_SERIES_IDX || cbxHydro1Type->ItemIndex == AG_SERIES_IDX ) {
		if ( edHydro1CellNo->Text.IsEmpty() )   bOk = false;
		if ( edHydro1CellSize->Text.IsEmpty() ) bOk = false;
		if ( edHydro1Blank->Text.IsEmpty() )    bOk = false;
		if ( !bOk ) {
			ShowMessage("유속계 1번 AG/SL 시리즈의 설정에 오류가 있습니다.");
			return false;
		}
	}
	...
	// EWSV Check
	bOk = true;
	if ( cbxHydro1Type->ItemIndex == EWSVCONVERTER_IDX ) {
		if ( cbxEWSVAvg->ItemIndex == -1 ) bOk = false;		// RADS의 평균 산출 방식 미지정
		if ( edEWSVWorkTime->Text.IsEmpty() )  bOk = false;	// 센서 동작 시간(초) 미지정
		if ( cbxEWSVSensorAvg->ItemIndex == -1 ) bOk = false;	// 센서 평균 산출 미지정
		if ( !bOk ) {
			ShowMessage("유속계 1번 EWSVConverter 설정에 오류가 있습니다.");
			return false;
		}
	}
	...
	// Hydro2 --------------------------------------------------
	bOk = true;
	if ( cbxHydro2Type->ItemIndex == -1 )     bOk = false;
	if ( cbxHydro2Type->ItemIndex > 0 ) {	// NONE이 아니면 포트, 속도 지정
		if ( cbxHydro2ComPort->ItemIndex <= 0 )  bOk = false;
		if ( cbxHydro2BaudRate->ItemIndex<= 0 ) bOk = false;
	}
	//if ( edHydro2Offset->Text.IsEmpty() )     return false;
	if ( cbxHydro2Reverse->ItemIndex == -1 )  bOk = false;
	if ( cbxHydro2IdleCheck->ItemIndex == -1 ) bOk = false;
	if ( cbxHydro2IdleCheckTime->ItemIndex == -1 ) bOk = false;
	//if ( edHydro2StartCell->Text.IsEmpty() )  bOk = false;
	//if ( edHydro2EndCell->Text.IsEmpty() )    bOk = false;
	if ( !bOk ) {
		ShowMessage("유속계 2번 설정에 오류가 있습니다.");
		return false;
	}

	// Check AG/SL Series
	bOk = true;
	if ( cbxHydro2Type->ItemIndex == SL_SERIES_IDX || cbxHydro2Type->ItemIndex == AG_SERIES_IDX ) {
		if ( edHydro2CellNo->Text.IsEmpty() )   bOk = false;
		if ( edHydro2CellSize->Text.IsEmpty() ) bOk = false;
		if ( edHydro2Blank->Text.IsEmpty() )    bOk = false;
		if ( !bOk ) {
			ShowMessage("유속계 2번 AG/SL 시리즈의 설정에 오류가 있습니다.");
			return false;
		}
	}
	...
	// Hydro3 --------------------------------------------------
	bOk = true;
	if ( cbxHydro3Type->ItemIndex == -1 )     bOk = false;
	if ( cbxHydro3Type->ItemIndex > 0 ) {	// NONE이 아니면 포트, 속도 지정
		if ( cbxHydro3ComPort->ItemIndex <= 0 )  bOk = false;
		if ( cbxHydro3BaudRate->ItemIndex <= 0 ) bOk = false;
	}
	//if ( edHydro2Offset->Text.IsEmpty() )     bOk = false;
	if ( cbxHydro3Reverse->ItemIndex == -1 )  bOk = false;
	if ( cbxHydro3IdleCheck->ItemIndex == -1 ) bOk = false;
	if ( cbxHydro3IdleCheckTime->ItemIndex == -1 ) bOk = false;
	//if ( edHydro2StartCell->Text.IsEmpty() )  bOk = false;
	//if ( edHydro2EndCell->Text.IsEmpty() )    bOk = false;
	if ( !bOk ) {
		ShowMessage("유속계 3번 설정에 오류가 있습니다.");
		return false;
	}

	// Check AG/SL Series
	bOk = true;
	if ( cbxHydro3Type->ItemIndex == SL_SERIES_IDX || cbxHydro3Type->ItemIndex == AG_SERIES_IDX ) {
		if ( edHydro3CellNo->Text.IsEmpty() )   bOk = false;
		if ( edHydro3CellSize->Text.IsEmpty() ) bOk = false;
		if ( edHydro3Blank->Text.IsEmpty() )    bOk = false;
		if ( !bOk ) {
			ShowMessage("유속계 3번 AG/SL 시리즈의 설정에 오류가 있습니다.");
			return false;
		}
	}
	...
	if ( cbxWaterType->ItemIndex > NONE_IDX ) {
		if ( cbxWaterComPort->ItemIndex <= NONE_IDX || cbxWaterBaudRate->ItemIndex <= NONE_IDX ) {
			ShowMessage("수위계의 통신포트, 통신속도 설정에 오류가 있습니다.");
			return false;
		}
		...
		if ( cxWaterAtmos->Checked ) {	// 대기압 센서 Enable 되어있으면 => 대기압 포트와 기준 대기압이 설정되어 있어야 한다.
			if ( cbxAtmosPort->ItemIndex <= NONE_IDX ) {
				ShowMessage("대기압 보정 기능이 Enable 되어 있으면 대기압 데이터 수집용 포트가 지정되어야 합니다.");
				return false;
			}
			if ( edBaseAtmos->Text.IsEmpty() ) {
				ShowMessage("대기압 보정 기능이 Enable 되어 있으면 기준 대기압이 입력되어야 합니다.");
				return false;
			}
		}

		bOk = true;
		if ( cbxWaterIdleCheck->ItemIndex == -1 ) bOk = false;
		if ( cbxWaterIdleCheckTime->ItemIndex == -1 ) bOk = false;
		if ( !bOk ) {
			ShowMessage("수위계의 Idle Check 옵션이 설정되지 않았습니다.");
			return false;
		}

		// 수위계가 RnsWater이고, 기상센서가 Enable되어 있으면 1번 유속계의 Airmar 설정이 Enable되어 있으면 안된다.
		if ( cbxWaterType->ItemIndex == RNSWATER_IDX ) {	// 수위계 RnsWater 이면
			if ( cxWaterAirmar->Checked ) {	// 기상센서를 수신한다면(Airmar/RMYoung)
				// 1번 유속계가 RQ-30d 이면 Airmar 설정이 살아 있으면 안된다.
				if ( cbxHydro1Type->ItemIndex == RQ_30D_IDX ) {	// 유속계 1번이 RQ-30d이면
					// Airmar Port르 Chek한다.
					if ( cbxAirmar1ComPort->ItemIndex > 0 ) {	// 유속계1번의 Airmar Port가 설정되면 안된다.
						ShowMessage("RnsWater 센서에서 기상센서 데이터를 수신하므로 1번 유속계에서 기상 데이터를 수신할 수 없습니다.");
						return false;
					} else {}
				} else {}
			} else {}
	...
	// HydroMeter1 check
	if ( cbxHydro1Type->ItemIndex == ACCUSONIC_A_IDX || cbxHydro1Type->ItemIndex == ACCUSONIC_B_IDX ) {	//  AccusonicA/AccusonicB이면 통신속도는 19200이어야 한다.
	...
	// HydroMeter2 check
	if ( cbxHydro2Type->ItemIndex == ACCUSONIC_A_IDX || cbxHydro2Type->ItemIndex == ACCUSONIC_B_IDX ) {	//  AccusonicA/AccusonicB이면 통신속도는 19200이어야 한다.
	...
	// RQ-30/RQ-30d Check
	if ( cbxHydro1Type->ItemIndex == RQ_30_IDX || cbxHydro1Type->ItemIndex == RQ_30D_IDX ) {	// RQ-30/RQ-30d
		//if ( cbxAirmar1ComPort->ItemIndex == -1 ) return false;	// 설정안함=> Error
		if ( cbxAirmar1ComPort->ItemIndex <= 0 ) {	// NONE, 이거나 설정이 없으면
			// 2026.02.13 RnsWater에서 Airmar를 수용하므로 RQ-30에서 굳이 Airmar를 수용해야 할 필요 없음
			// 이미 Airmar와 같이 운영되는 사이트가 있어어, 설정을 못하게 하는 것은 아니지만 설정 체크사항에서 뺌.
...
//			}
			...
			// Airmar Port 지정된 상태 => RQ-30d에서 Airmar를 수용하겠다는 뜻
			if ( cbxWaterType->ItemIndex == RNSWATER_IDX && cxWaterAirmar->Checked ) {	// 수위계가 RnsWater 이고, 기상 데이터 Enable이면
				ShowMessage("유속계 1번에서 기상센서 설정과 수위계(RnsWater)의 기상센서 설정이 중복됩니다.");
				return false;
			} else {
				if ( cbAirmarLinker->Checked ) {	// AirmarLinker를 통해 통신, Airmar 바로 통신 => 4800bps
					if ( cbxAirmar1BaudRate->ItemIndex != 3 ) { // Index => 3 => 9600bps ; AirmarLinker는 9600bps로 통신
						ShowMessage("AirmarLinker를 이용할 경우 통신 속도는 9600bps로 설정 바랍니다.");
						return false;
					} else {}
				} else {	// Airmar Sensor와 바로 통신
					if ( cbxAirmar1BaudRate->ItemIndex != 2 ) { // Index => 2 => 4800bps ; Airmar sensor는 4800bps로 통신
						ShowMessage("Airmar sensor와 바로 연결된다면 통신속도는 4800bps입니다.");
						return false;
					} else {}
				}
				if ( cxWaterAirmar->Checked ) { 	// 수위계에서 Airmar를 수신한다면
					ShowMessage("유속계(RQ-30d)에서 Airmar를 수용하면 수위계(RnsWater)에서 기상 데이터를 수신할 수 없습니다. ");
					return false;
				}
			}
		}
	...
	// RQ-30d Check
	if ( cbxHydro1Type->ItemIndex == RQ_30D_IDX ) {	// RQ-30d
		// 수위계의 기상 센서의 Check ---------------------------------------------------
		if ( cbxAirmar1ComPort->ItemIndex > 0 ) {	// 기상 센서의 포트가 지정되어 있다면
			// 수위계에서 기상 센서 데이터를 수신하면 안된다.
			if ( cxWaterAirmar->Checked ) {
				ShowMessage("1변 유속계(RQ-30d)에서 기상데이터를 수집하므로 수위계서 기상데이터를 수집할 수 없습니다.");
				return false;
			} else {}
		} else {}

		// Slave Device Check (Slave Count) ------------------------------------
		if ( cbxSlaveCount->ItemIndex == -1 ) {
			ShowMessage("Slave의 수를 지정하지 않았습니다.");
			return false;
		}
	...
	// 수위 데이터 사용 구분 ---------------------------------------------------
	if ( cbxWaterUseFlag->ItemIndex == -1 ) {
		ShowMessage("수위 데이터 사용 구분을 설정 바랍니다.");
		return false;
	}

	if ( cbxWaterUseFlag->ItemIndex == 0 ) {	// 유속계 1번의 수위 데이터 사용: RQ-30, RQ-30d, EWSVConverter일 때만 수위 사용 가능
		if ( cbxHydro1Type->ItemIndex == RQ_30_IDX ||cbxHydro1Type->ItemIndex == RQ_30D_IDX ||
			 cbxHydro1Type->ItemIndex == EWSVCONVERTER_IDX ) {
			// RQ-30, RQ-30d, EWSVConverter의 수위 데이터를 사용하겠다는 설정.. ok
		} else {
			ShowMessage("수위 데이터 사용 구분은 유속계 1로 되어 있으나 유속계 1이 RQ-30, RQ-30d, EWSVConverter 일 때만 수위를 사용할 수 있습니다.");
			return false;
		}
	} else {	// 수위 데이터의 수위 사용
		if ( cbxWaterType->ItemIndex > 0 ) {	// 수위계 장비가 설정되어 있으면.. ok
			// 수위계의 수위 데이터를 사용하겠다 => ok
		} else {
			// 수위계를 설정하지 않은 상태
			ShowMessage("수위 데이터 사용 구분은 수위계지만 수위계 장비 설정이 없습니다.");
			return false;
		}
	}
...
bool TFormSetup::LoadLoggerFromDB()
{
	TSQLQuery *pQuery = NULL;
	TRSetup Setup;
	TRSetup FlowStream, LoggerInfo, HydroMeter1, HydroMeter2, HydroMeter3, WaterLevel;
	UnicodeString sSQL = " SELECT * FROM RSetup               "
	...
	while ( !pQuery->Eof ) {
		try {
			...
			if ( Setup.PK1 == 10 && Setup.PK2 == 5 ) HydroMeter3 = Setup;
	...
	if ( cbxHydro1Type->ItemIndex == NONE_IDX ) { 	// NONE
		Panel1->Visible      = false;	// SL/AG Series
		PanelAirmar->Visible = false;	// Airmar
		PanelRQ30d->Visible  = false;	// RQ-30d
		PanelEWSV->Visible   = false;
	} else if( cbxHydro1Type->ItemIndex == SL_SERIES_IDX || cbxHydro1Type->ItemIndex == AG_SERIES_IDX ) {
		Panel1->Visible      = true;	// SL/AG Series
		PanelAirmar->Visible = false;
		PanelRQ30d->Visible  = false;
		PanelEWSV->Visible   = false;
	} else if ( cbxHydro1Type->ItemIndex == RQ_30_IDX ) {	// RQ-30
		Panel1->Visible      = false;	// SL/AG Series
		PanelAirmar->Visible = true;	// Airmar
		PanelRQ30d->Visible  = false;
		PanelEWSV->Visible   = false;
	} else if ( cbxHydro1Type->ItemIndex == RQ_30D_IDX ) {	// RQ-30d
		Panel1->Visible      = false;	// SL/AG Series
		PanelAirmar->Visible = true;	// Airmar
		PanelRQ30d->Visible  = true;
		PanelEWSV->Visible   = false;
	} else if ( cbxHydro1Type->ItemIndex == EWSVCONVERTER_IDX ) {	// EWSVConverter
		Panel1->Visible      = false;	// SL/AG Series
		PanelAirmar->Visible = false;	// Airmar
		PanelRQ30d->Visible  = false;
		PanelEWSV->Visible   = true;	// EWSVConverter Show
	} else {
	...
	} else if ( HydroMeter1.Value01 == EWSVCONVERTER_STR ) {	// EWSVConverter
		cbxEWSVAvg->ItemIndex       = (HydroMeter1.Value18.IsEmpty()) ? 1 : ((HydroMeter1.Value18 == "AVG") ? 0 : 1);	// 0:AVG, 1:MID(default)
		edEWSVWorkTime->Text        = (HydroMeter1.Value19.IsEmpty()) ? UnicodeString("230") : HydroMeter1.Value19;	// Default: 230
		cbxEWSVSensorAvg->ItemIndex = (HydroMeter1.Value20.IsEmpty()) ? 1 : ((HydroMeter1.Value18 == "AVG") ? 0 : 1);	// 0:AVG, 1:MID(default)
	} else {
		// nothing to do..
	}
	...
	//heaven2 2022.03.18
	if ( HydroMeter1.Value01 == CHANNELMASTER_STR || HydroMeter1.Value01 == AGSERIES_STR ||   // 유효셀은 초음파 유속계에만 해당
		 HydroMeter1.Value01 == SLSERIES_STR ) {
	...
	if ( cbxHydro2Type->ItemIndex == NONE_IDX ) { // NONE
		Panel2->Visible       = false;	// SL/AG Series
	} else if( cbxHydro2Type->ItemIndex == SL_SERIES_IDX || cbxHydro2Type->ItemIndex == AG_SERIES_IDX ) {
		Panel2->Visible       = true;	// SL/AG Series
		//PanelAirmar2->Visible = false;
	} else if ( cbxHydro2Type->ItemIndex == RQ_30_IDX ) {	// RQ-30
		Panel2->Visible       = false;	// SL/AG Series
...
//		cbxAirmar2BaudRate->ItemIndex = cbxAirmar2BaudRate->Items->IndexOf(HydroMeter2.Value17);	// BaudRate
	...
	UnicodeString HydroType3 = HydroMeter3.Value01;
	if ( HydroMeter3.Value01 == RQ30_STR )  HydroType3 = "RQ-30";
	if ( HydroMeter3.Value01 == RQ30D_STR ) HydroType3 = "RQ-30d";
	cbxHydro3Type->ItemIndex     = cbxHydro3Type->Items->IndexOf(HydroType3);
	if ( cbxHydro3Type->ItemIndex == NONE_IDX ) { // NONE
		Panel3->Visible       = false;	// SL/AG Series
	} else if( cbxHydro3Type->ItemIndex == SL_SERIES_IDX || cbxHydro3Type->ItemIndex == AG_SERIES_IDX ) {
		Panel3->Visible       = true;	// SL/AG Series
	} else if ( cbxHydro2Type->ItemIndex == RQ_30_IDX ) {	// RQ-30
		Panel3->Visible       = false;	// SL/AG Series
	} else {
		Panel3->Visible       = false;	// SL/AG Series
	...
	cbxHydro3ComPort->ItemIndex  = cbxHydro3ComPort->Items->IndexOf(HydroMeter3.Value02);
	cbxHydro3BaudRate->ItemIndex = cbxHydro3BaudRate->Items->IndexOf(HydroMeter3.Value03);
	edHydro3Offset->Text         = HydroMeter3.Value04;
	cbxHydro3Reverse->ItemIndex  = (HydroMeter3.Value10 == "OFF")  ? 1 : 0;
	cbxHydro3UpSide->ItemIndex   = (HydroMeter3.Value11 == "SIDE") ? 1 : 0;
	if ( HydroMeter3.Value01 == AGSERIES_STR || HydroMeter3.Value01 == SLSERIES_STR ) {
		edHydro3CellNo->Text   = HydroMeter3.Value07;
		edHydro3CellSize->Text = HydroMeter3.Value08;
		edHydro3Blank->Text    = HydroMeter3.Value09;
	} else if ( HydroMeter3.Value01 == RQ30_STR ) {
//		cbxAirmar2ComPort->ItemIndex  = cbxAirmar2ComPort->Items->IndexOf(HydroMeter2.Value16); 	// Port
//		cbxAirmar2BaudRate->ItemIndex = cbxAirmar2BaudRate->Items->IndexOf(HydroMeter2.Value17);	// BaudRate
	} else {
		// nothing to do..
	}
	edHydro3Hz->Text             = HydroMeter3.Value12;
	edHydro3SN->Text             = HydroMeter3.Value13;
	cbxHydro3IdleCheck->ItemIndex = (HydroMeter3.Value14.IsEmpty() || HydroMeter3.Value14 == "ON" ) ? 0 : 1; // ON/OFF
	cbxHydro3IdleCheckTime->ItemIndex = (HydroMeter3.Value15.IsEmpty()) ? 2 : (StrToInt(HydroMeter3.Value15)-1); // 1 hour ~ 6 hours (default) 3 hours
	//heaven2 2022.03.18
	edHydro3StartCell->Text      = (HydroMeter3.Value18.IsEmpty()) ? UnicodeString("1") : HydroMeter3.Value18;	// Default : 1(
	edHydro3EndCell->Text        = (HydroMeter3.Value19.IsEmpty()) ? UnicodeString("10") : HydroMeter3.Value19;	// Default : 1(
	...
	if ( WaterLevel.Value01.Pos(LT400_STR) > 0 ) { // LT400
		cbxWaterType->ItemIndex = LT400_ATMOS_IDX; // LT400+Atmos
		...
		lbAtmosEnable->Visible  = true;	// (대기압)대기압 보정 유무 Caption
		cxWaterAtmos->Visible   = true;	// (대기압)대기압 보정 유무
		cxWaterAtmos->Checked   = (WaterLevel.Value11 == "ON") ? true : false;
		lbAtmosCap->Visible     = true;	// (대기압)대기압센서 포트 Caption
		cbxAtmosPort->Visible   = true;	// (대기압)대기압센서 포트
		edBaseAtmos->Visible    = true;	// (대기압)기준대기압

	} else if ( WaterLevel.Value01.Pos(RNSWATER_STR) > 0 ) {	// RnsWater
		cbxWaterType->ItemIndex = RNSWATER_IDX; 	// RnsWater
		if ( WaterLevel.Value07.IsEmpty() ) WaterLevel.Value07 = "NONE";	// Init Value
		cbxAtmosPort->ItemIndex = cbxAtmosPort->Items->IndexOf(WaterLevel.Value07);
		edBaseAtmos->Text       = WaterLevel.Value08;
		lbAtmosEnable->Visible  = true;	// (대기압)대기압 보정 유무 Caption
		...
		// Airmar
		lbAirmarCap->Visible    = true;	// (기상센서) Enable
		cxWaterAirmar->Checked  = (WaterLevel.Value12.Pos("ON") > 0 ) ? true : false;	// ON : Checked, OFF: UnChecked
	} else { // 다른 장비 보여줄 때 감춘다.
		cbxAtmosPort->ItemIndex = 0; 	// NONE
		edBaseAtmos->Text       = 0;
		lbAtmosEnable->Visible  = false;	// (대기압)대기압 보정 유무 Caption
		cxWaterAtmos->Visible   = false;	// (대기압)대기압 보정 유무
		lbAtmosCap->Visible     = false;	// (대기압)대기압센서 포트 Caption
		cbxAtmosPort->Visible   = false;	// (대기압)대기압센서 포트
		edBaseAtmos->Visible    = false;	// (대기압)기준대기압

		// Airmar
		lbAirmarCap->Visible    = false;	// (기상센서) Disable
		cxWaterAirmar->Checked  = false;	// ON : Checked, OFF: UnChecked
	}
	...
	cbxWaterUseFlag->ItemIndex = (WaterLevel.Value13.IsEmpty()) ? 1 : ((WaterLevel.Value13 == HYDROMETER_STR) ? 0 : 1);
								// Default: 수위계 수위 사용, HydroMeter로 지정되면 유속계1에서 사용 0: HydroMeter, 1: WaterLevel
...
//	}
	...
	if ( HydroMeter1.Value01 == AGSERIES_STR || HydroMeter1.Value01 == SLSERIES_STR ) {
		...
		HydroMeter1.Value18 = (cbxEWSVAvg->ItemIndex == 0) ? "AVG" : "MID";	        // AVG: 평균, MID:중간값
		HydroMeter1.Value19 = edEWSVWorkTime->Text;	                                // 센서 동작 시간, Default(230초 3분 50초)
		HydroMeter1.Value20 = (cbxEWSVSensorAvg->ItemIndex == 0 ) ? "AVG" : "MID";	// 센서의 값을 취하는 방법 : AVG: 평균, MID:중간값
	} else {
		HydroMeter1.Value05 = "";	// not use
		HydroMeter1.Value06 = "";	// not use
	...
	if ( HydroMeter1.Value01 == RQ30_STR || HydroMeter1.Value01 == RQ30D_STR ) {	// RQ-30 시리즈이면 Airmar 설정 필요
	...
	if ( HydroMeter1.Value01 == CHANNELMASTER_STR || HydroMeter1.Value01 == AGSERIES_STR ||	// 초음파 유속계이면 유효셀 범위 지정
		 HydroMeter1.Value01 == SLSERIES_STR ) {
...
//	HydroMeter2.Value17 = cbxAirmar2BaudRate->Text;		// Airmar BaudRate
	...
	HydroMeter3.PK1 = 10;
	HydroMeter3.PK2 = 5;	// 주의 PK2 = 5, 4는 수위계

	HydroMeter3.Value01 = cbxHydro3Type->Text;
	if ( cbxHydro3Type->Text == "RQ-30")  HydroMeter3.Value01 = RQ30_STR;
	if ( cbxHydro3Type->Text == "RQ-30d") HydroMeter3.Value01 = RQ30D_STR;
	HydroMeter3.Value02 = cbxHydro3ComPort->Text;
	HydroMeter3.Value03 = cbxHydro3BaudRate->Text;
	HydroMeter3.Value04 = edHydro3Offset->Text;

	HydroMeter3.Value07 = edHydro3CellNo->Text;
	HydroMeter3.Value08 = edHydro3CellSize->Text;
	HydroMeter3.Value09 = edHydro3Blank->Text;

	HydroMeter3.Value10 = cbxHydro3Reverse->Text;
	HydroMeter3.Value11 = cbxHydro3UpSide->Text;

	HydroMeter3.Value12 = edHydro3Hz->Text;
	HydroMeter3.Value13 = edHydro3SN->Text;

	HydroMeter3.Value14 = (cbxHydro3IdleCheck->ItemIndex == -1 || cbxHydro3IdleCheck->ItemIndex == 0) ? "ON" : "OFF";  // default : ON => idle check ON
	HydroMeter3.Value15 = (cbxHydro3IdleCheckTime->ItemIndex == -1) ? 2 : (cbxHydro3IdleCheckTime->ItemIndex + 1); // default : 3 hours

//	HydroMeter3.Value16 = cbxAirmar2ComPort->Text;		// Airmar ComPort
//	HydroMeter3.Value17 = cbxAirmar2BaudRate->Text;		// Airmar BaudRate

	HydroMeter3.Value18 = ((edHydro3StartCell->Text).IsEmpty()) ? UnicodeString("1") :  edHydro3StartCell->Text;
	HydroMeter3.Value19 = ((edHydro3EndCell->Text).IsEmpty())   ? UnicodeString("10") :  edHydro3EndCell->Text;

	bResult = HydroMeter3.Update();
	if ( !bResult ) {
		ShowMessage("3번 유속계  정보 저장 시 오류가 발생하였습니다.");
		return false;
	}
	...
	WaterLevel.Value07 = cbxAtmosPort->Text;	// 대기압 Port
	if ( WaterLevel.Value07.IsEmpty() ) WaterLevel.Value07 = "NONE";	// INIT Value
	...
	WaterLevel.Value11 = (cxWaterAtmos->Checked)  ? "ON" : "OFF";	// 대기압 보정 유무	(ON/OFF)
	WaterLevel.Value12 = (cxWaterAirmar->Checked) ? "ON" : "OFF";	// Airmar 정보 수집 유무 ( ON/OFF)

	WaterLevel.Value13 = (cbxWaterUseFlag->ItemIndex == 0) ? HYDROMETER_STR : WATERLEVEL_STR;	// 수위 데이터 : 유속계/수위계 구분
...
{
	// 유속계 1 : NONE 선택 시
	if ( cbxHydro1Type->ItemIndex == NONE_IDX ) {
		cbxHydro1ComPort->ItemIndex = NONE_IDX;
		cbxHydro1BaudRate->ItemIndex = NONE_IDX;
	}
	...
	if ( cbxHydro1Type->ItemIndex == 0 ) {	// NONE
		...
		PanelRQ30d->Visible      = true;
		PanelEWSV->Visible       = false;
	} else if ( cbxHydro1Type->ItemIndex == 8 ) { // EWSVConverter  => 부속 Panel 모두 감춘다.
		Panel1->Visible          = false;
		PanelAirmar->Visible     = false;
		PanelRQ30d->Visible      = false;
		PanelEWSV->Visible       = true;	// EWSV Panel만 View 시킨다.
...
{
	// 유속계 1 : NONE 선택 시
	if ( cbxHydro2Type->ItemIndex == NONE_IDX ) {
		cbxHydro2ComPort->ItemIndex = NONE_IDX;
		cbxHydro2BaudRate->ItemIndex = NONE_IDX;
	}
	...
	} if ( cbxHydro2Type->ItemIndex == SL_SERIES_IDX || cbxHydro2Type->ItemIndex == AG_SERIES_IDX ) { // SL/AG Series
		Panel2->Visible       = true;
		//PanelAirmar2->Visible = false;
	} else {
...
void __fastcall TFormSetup::cbxWaterTypeChange(TObject *Sender)
{
	// 수위계 종류 : NONE을 선택하면
	if ( cbxWaterType->ItemIndex == NONE_IDX ) {
		cbxWaterComPort->ItemIndex = NONE_IDX;
		cbxWaterBaudRate->ItemIndex = NONE_IDX;
	}

	// LT400, RnsWater => 대기압 Enable / Disable
	if ( cbxWaterType->ItemIndex == LT400_ATMOS_IDX || cbxWaterType->ItemIndex == RNSWATER_IDX ) {	// Type => 2: LT400+Atmos, 5: RnsWater )
		lbAtmosEnable->Visible = true;	// (대기압) 대기압 보정 유무 Caption
		cxWaterAtmos->Visible  = true;	// (대기압)대기압 보정 유무
		lbAtmosCap->Visible    = true;	// (대기압)대기압센서 포트 Caption
		cbxAtmosPort->Visible  = true;	// (대기압)대기압센서 포트
		edBaseAtmos->Visible   = true;	// (대기압)기준대기압
		cbxAtmosPort->ItemIndex = ConvertComPort(theEnv.m_AtmosPort);	// (대기압) 대기압 포트 설정
	} else {
		lbAtmosEnable->Visible = false;	// (대기압) 대기압 보정 유무 Caption
		cxWaterAtmos->Visible  = false;	// (대기압)대기압 보정 유무
		lbAtmosCap->Visible    = false;	// (대기압)대기압센서 포트 Caption
	...
	// RnsWater일 때 Airmar 데이터 수용 여부
	if ( cbxWaterType->ItemIndex == RNSWATER_IDX ) {	// Type => 5: RnsWater Type => Airmar Enable/Disable
		lbAirmarCap->Visible   = true;	// (기상센서) Enable/Disable Caption
		cxWaterAirmar->Visible = true;  // (기상센서) Enable/Disable
	} else {
		lbAirmarCap->Visible   = false;	// (기상센서) Enable/Disable Caption
		cxWaterAirmar->Visible = false;	// (기상센서) Enable/Disable
	}

	if ( cbxWaterType->ItemIndex == RNSWATER_IDX) {	// Sensor Type => 5 (RnsWater) => AirmarLinker 설정 중복 여부
		// AirmarLinker 설정을 살표본다.
		if ( cbxAirmar1ComPort->ItemIndex > 0 ) {
			if ( Application->MessageBox(L"RnsWaterr는 AirmarLinker의 기능을 포함합니다. AirmarLinker의 설정은 무시됩니다.",
										 L"AirmarLinker경고", MB_YESNO) == IDYES ) {
				// AirmarLinker 옵션 무시 진행 ok
...
void __fastcall TFormSetup::cxWaterAirmarClick(TObject *Sender)
{
	if ( cxWaterAirmar->Checked ) {
		//ShowMessage("RnsWater 센서에서 기상센서의 데이터를 함께 수집합니다.");
	} else {	// Off
		//ShowMessage("RNSWATER 센서에서 기상센서의 데이터를 수집하지 않습니다.");
	}
}
//---------------------------------------------------------------------------

void __fastcall TFormSetup::cxWaterAtmosClick(TObject *Sender)
{
	if ( cxWaterAtmos->Checked ) {
		//ShowMessage("수위의 대기압 보정 기능이 Enable 되었습니다. 대기압 수집을 위한 포트와 기준 대기압을 입력하세요.");
	} else {
		//ShowMessage("수위의 대기압 보정 기능이 Disable 되었습니다.");
	}
}
//---------------------------------------------------------------------------

void __fastcall TFormSetup::cbxHydro3TypeChange(TObject *Sender)
{
	// 유속계 1 : NONE 선택 시
	if ( cbxHydro3Type->ItemIndex == NONE_IDX ) {
		cbxHydro3ComPort->ItemIndex = NONE_IDX;
		cbxHydro3BaudRate->ItemIndex = NONE_IDX;
	}

	if ( cbxHydro3Type->ItemIndex == NONE_IDX ) { 	// NONE
		Panel3->Visible          = false;	// SL/AG Option
	} if ( cbxHydro3Type->ItemIndex == SL_SERIES_IDX || cbxHydro3Type->ItemIndex == AG_SERIES_IDX ) { // SL/AG Series
		Panel3->Visible       = true;
	} else {
...
void __fastcall TFormSetup::cbxAirmar1ComPortChange(TObject *Sender)
{
	if ( cbxAirmar1ComPort->ItemIndex == NONE_IDX ) {
		cbxAirmar1BaudRate->ItemIndex = NONE_IDX;
		cbAirmarLinker->Checked = false;
	} else {
    	// nothing to do
	}
}
