...
//---------------------------------------------------------------------------
// Init
void TRQ30::Init(TApdComPort *pPort, EN_HYDRO_NO enHydroNo)
{
	m_pPort  = pPort;
...
bool TRQ30::Save(UnicodeString MeasureDate, UnicodeString MeasureTime)
{
	...
	CaculateRQ30DataAvg();		// 2026.03.24 heaven2
...
//---------------------------------------------------------------------------
// RQ30 데이터 중에 평균해서 사용한다. 2026.03.24
bool TRQ30::CaculateRQ30DataAvg()
{
	int Count = 0;
	Count = m_pRQ30List->Count;
	...
	for ( int i = 0; i < m_pRQ30List->Count ; i++ ) {
		...
		if ( pRQ30Data != NULL ) {
			ItemCount++;
			// SelfCheck 코드는 가장 높은 수를 남긴다.
			xData.SelfCheck      = ((pRQ30Data->SelfCheck > xData.SelfCheck ) ? pRQ30Data->SelfCheck : 0);
			xData.Waterlevel     += pRQ30Data->Waterlevel;
			xData.Velocity       += pRQ30Data->Velocity;
			// SNR은 평균하되, 소수점 이하는 가장 높은 수를 남긴다.
			double x1 = xData.Quality  - (int)xData.Quality;	// 소수점 이하를 남긴다
			double x2 = pRQ30Data->Quality - (int)pRQ30Data->Quality;	// 소수점 이하를 남긴다.
			xData.Quality        += (int)xData.Quality + (int)pRQ30Data->Quality;
			xData.Quality        += ((x2 > x1 ) ? x2 : x1);
			//
			xData.Discharge      += pRQ30Data->Discharge;
			xData.Area           += pRQ30Data->Area;
			xData.LearnVelocity  += pRQ30Data->LearnVelocity;
			xData.LearnDischarge += pRQ30Data->LearnDischarge;
			xData.OppositeContent+= pRQ30Data->OppositeContent;
			xData.SupplyVoltage  += pRQ30Data->SupplyVoltage;
			xData.PeakWidth      += pRQ30Data->PeakWidth;
			xData.CSR            += pRQ30Data->CSR;
			xData.AreaPeak       += pRQ30Data->AreaPeak;
			xData.RmsPic         += pRQ30Data->RmsPic;
			xData.Amp            += pRQ30Data->Amp;
			xData.AmpRelation    += pRQ30Data->AmpRelation;
			xData.Signal         += pRQ30Data->Signal;
			// 에러 코드도 가장 높은 에러코드를 남긴다.
			xData.ErrorCode       = ((pRQ30Data->ErrorCode > xData.ErrorCode) ? pRQ30Data->ErrorCode : xData.ErrorCode);
			xData.Angle          += pRQ30Data->Angle;
			xData.Complete        = pRQ30Data->Complete;
		}
	}
	...
	if ( ItemCount > 0 ) {
		...
		xData.Area           = xData.Area            / ItemCount;
