...
TEnv::TEnv()
{
	...
	// HydroMeter3
	m_HydroType3     = "NONE";		// 하층유속계 Type(ChannelMaster, SL Series, AG Series )
	m_HydroPort3     = "NONE";		// COM1~COM32
	m_HydroBaudRate3 = 0;			// 통신속도(9600)
	m_HydroOffset3   = 0;			// Offset
	m_HydroHz3       = "";			// 동작주파수
	m_HydroSN3       = "";			// 일련번호
	m_StartCell3     = 1;           // 유효셀 시작 번호(Default: 1) - 2022.03.18
	m_EndCell3       = 10;          // 유효셀 끝 번호(Default: 10)  - 2022.03.18

	m_CellBegin3     = 0;			// Cell Begin
	m_CellEnd3       = 0;			// Cell End
	m_CellNumber3    = 0;			// Cell Number
	m_CellSize3      = 0;			// Cell Size;
	m_CellBlank3     = 0;			// Cell Blank

	m_bIdleCheck3    = true;		// idle Check flag
	m_iIdleCheckTime3 = 3;		// idle check time => 180 mintues => 3 hours
	...
	m_bIdleCheckWater     = false;		// idle Check flag
	m_iIdleCheckWaterTime = 3;		// idle check time => 180 mintues => 3 hours

	m_bAtmosEnable   = false;	// 대기압 보정 ON/OFF ( 10초 단위 지속 수집)
	m_bAirmarEnable  = false;	// 기상 센서 데이터 수집 ON/OFF => 1분 단위 데이터 수집(누적 없고, 현재 상태만)

	m_WaterUseFlag   = WATERLEVEL_STR;	// "HydroMeter" / "WaterLevel" 수위 데이터 사용 구분
...
bool TEnv::LoadEnv()
{
	...
	while ( !pQuery->Eof ) {
		try {
			...
			if ( Setup.PK1 == 1 && Setup.PK2 == 1 ) {          	// 지점정보
			...
			} else if ( Setup.PK1 == 10 && Setup.PK2 == 2 ) {	// HydroMeter1 하층 유속계 정보
				m_HydroType1      = Setup.Value01;				// 하층유속계 Type(ChannelMaster, SL Series, AG Series )
				m_HydroPort1      = Setup.Value02;				// COM1~COM32
				m_HydroBaudRate1  = (Setup.Value03.IsEmpty() || Setup.Value03 == NONE_STR ) ? 0 : StrToInt(Setup.Value03);				// 통신속도(9600)
				...
				} else if ( m_HydroType1 == EWSVCONVERTER_STR ) {
					m_EWSVAvg            = (Setup.Value18.IsEmpty()) ? UnicodeString("AVG") : Setup.Value18;	  // RADS에서 유속/수위의 평균 방법:AVG: 평균, MID: 중간값 ( Default: AVG)
					m_EWSVWorkTime       = Setup.Value19.ToIntDef(230);		// EWSV에 수용된 센서의 동작 시간(Default: 230초(4분), 1분: 50초, .. Idle time 10ch 여유
					m_EWSVSensorAvg      = (Setup.Value20.IsEmpty()) ? UnicodeString("AVG") : Setup.Value20;		            // 센서의 수위/유속의 평균 방법: AVG: 평균, MID: 중간값	( Default: AVG)
				} else {
					// nothing to to..
				}
				...
				m_HydroAirmarBaudRate1 = (Setup.Value17.IsEmpty() || Setup.Value17 == NONE_STR ) ? 4800 : ((Setup.Value17 == "NONE") ? 0 : StrToInt(Setup.Value17)); // 4800(default), 9600,....
				m_AirmarLinker     = (Setup.Value20.IsEmpty()) ? false : ((Setup.Value20 == "ON" ) ? true : false); //heaven2 2022.12.01
				if (  m_HydroType1 == CHANNELMASTER_STR || m_HydroType1 == AGSERIES_STR || m_HydroType1 == SLSERIES_STR ) {
					...
					m_StartCell1       = 1;	// 유효셀 시작 번호(Default:1)
					m_EndCell1         = 10;	// 유효셀 끝번호(Default:10)
				}
				...
				m_HydroBaudRate2  = (Setup.Value03.IsEmpty() || Setup.Value03 == NONE_STR) ? 0 : StrToInt(Setup.Value03);				// 통신속도(9600)
				...
				m_HydroAirmarBaudRate2 = (Setup.Value17.IsEmpty() || Setup.Value17 == NONE_STR) ? 4800 : StrToInt(Setup.Value17); // 4800(default), 9600,....
			...
			} else if ( Setup.PK1 == 10 && Setup.PK2 == 5 ) {	// 3번 유곡계 정보
				m_HydroType3      = Setup.Value01;				// 상층유속계 Type(ChannelMaster, SL Series, AG Series )
				m_HydroPort3      = Setup.Value02;				// COM1~COM32
				m_HydroBaudRate3  = (Setup.Value03.IsEmpty() || Setup.Value03 == NONE_STR) ? 0 : StrToInt(Setup.Value03);				// 통신속도(9600)
				m_HydroOffset3    = (Setup.Value04.IsEmpty()) ? 0 : StrToFloat(Setup.Value04);	// Offset
				m_CellBegin3      = (Setup.Value05.IsEmpty()) ? 0 : StrToFloat(Setup.Value05);	// Cell Begin
				m_CellEnd3        = (Setup.Value06.IsEmpty()) ? 0 : StrToFloat(Setup.Value06);	// Cell End
				m_CellNumber3     = (Setup.Value07.IsEmpty()) ? 0 : StrToFloat(Setup.Value07);	// Cell Number
				m_CellSize3       = (Setup.Value08.IsEmpty()) ? 0 : StrToFloat(Setup.Value08);	// Cell Size;
				m_CellBlank3      = (Setup.Value09.IsEmpty()) ? 0 : StrToFloat(Setup.Value09);	// Cell Blank
				m_HydroHz3        = Setup.Value12;  // Frequency
				m_HydroSN3        = Setup.Value13;  // Serial Number
				m_bIdleCheck3     = (Setup.Value14.IsEmpty()) ? true : ((Setup.Value14 == "ON") ? true : false);
				m_iIdleCheckTime3 = (Setup.Value15.IsEmpty()) ? 3  : StrToInt(Setup.Value15);	// 180minutes or thers
				m_StartCell3       = (Setup.Value18.IsEmpty()) ?  1 : StrToInt(Setup.Value18);	// 유효셀 시작 번호(Default:1)
				m_EndCell3         = (Setup.Value19.IsEmpty()) ? 10 : StrToInt(Setup.Value19);	// 유효셀 끝번호(Default:10)			} else if ( Setup.PK1 == 10 && Setup.PK2 == 4 ) {	// 수위계 정보
				...
				m_WaterBaudRate   = (Setup.Value03.IsEmpty() || Setup.Value03 == NONE_STR) ? 0 : StrToInt(Setup.Value03);	// BaudRate
				...
				m_AtmosPort       = Setup.Value07;	// Atmospheric Port if WaterLevel is LT400 2015.11.16, COM01,COM02,..
				m_BaseAtmos       = (Setup.Value08.IsEmpty()) ? 0 : StrToFloat(Setup.Value08); // 기준대기압
				m_WaterUseFlag    = (Setup.Value20.IsEmpty()) ? UnicodeString(WATERLEVEL_STR) : Setup.Value20;	// "HydroMeter" / "WaterLevel" Default: WaterLevel	2026.03.11
				...
				m_bAtmosEnable    = (Setup.Value11.IsEmpty()) ? false :	((Setup.Value11 == "ON") ? true : false);	// 2024.12.31 대기압 보정 유무
				m_bAirmarEnable   = (Setup.Value12.IsEmpty()) ? false : ((Setup.Value12 == "ON") ? true : false);	// 2024.12.31 기상센서 수집 유무
			} else {
				//nothing to do..
			}
